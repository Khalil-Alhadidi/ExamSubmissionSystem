...
public static class ExamEndpoints
{
    public static IEndpointRouteBuilder MapExamEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        v1.MapGet("/{id}", async (Guid id, GetExamConfigByIdHandler handler) =>
        {
        ...
        v1.MapGet("/{id}/with-questions", async (Guid id, GetExamWithQuestionsHandler handler) =>
        {
            var result = await handler.HandleAsync(id);
            return result is null ? Results.NotFound() : Results.Ok(result);
        })
        ...
        v1.MapPut("/{id}", async (Guid id, UpdateExamConfigDto dto, UpdateExamConfigHandler handler) =>
        {
